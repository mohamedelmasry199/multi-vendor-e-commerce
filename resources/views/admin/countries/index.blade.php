    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        ...
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            ...
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <!--begin::Title-->
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Countries') }}</h1>
                <!--end::Title-->
                <!--begin::Breadcrumb-->
                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-title">
                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <!--end::Svg Icon-->
                            <input type="text" id="search-country" class="form-control form-control-solid w-250px ps-14"
...
{{-- <div class="d-flex align-items-center">
    ...
    <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
        ...
        <form id="filterForm" action="{{ route('admin.countries.filter') }}" method="GET">
            <div class="px-7 py-5">
                <div class="mb-10">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="parent_only" name="parent_only">
                        <label class="form-check-label" for="parent_only">
                            {{ __('Countries with Parents') }}
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="children_only" name="children_only">
                        <label class="form-check-label" for="children_only">
                            {{ __('Countries with Children') }}
                        </label>
                    </div>
                </div>
...
<!--end::Modal-->
                            ...
                            <button type="button" class="btn btn-primary"
                                onclick="window.location='{{ route('admin.countries.create') }}';">
                ...
                <div class="card-body py-4">
                    ...
                    <div class="table-responsive">
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_countries">
                            <!-- Table Head -->
                            <thead>
                                <!-- Table Row -->
                                    ...
                                    <th class="w-10px pe-2">
                                        <div class="form-check form-check-sm form-check-custom form-check-solid me-3">
                                            <input class="form-check-input" type="checkbox" data-kt-check="true"
                                                data-kt-check-target="#kt_table_countries .form-check-input"
                                                value="1" />
                                        </div>
                                    </th>
                                    <th class="min-w-125px">{{ __('Name') }}</th>
                                    <th class="min-w-100px">{{ __('Flag') }}</th>
                                    ...
                                    <tr>
                                        ...
                                        <td class="d-flex align-items-center">
                                            <div class="d-flex flex-column">
                                                <a href="{{ route('admin.countries.show', ['country' => $country->id]) }}"
                                                    class="text-gray-800 text-hover-primary mb-1">{{ $country->name_ar }}</a>
                                        ...
                                        <td> <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
                                            <a href="{{ route('admin.countries.show', ['country' => $country->id]) }}">
                                                <div class="symbol-label">
                                                    <img src="{{ $country->getImgPath('flag') }}" alt="{{ $country->name_ar }}"
                                                        class="w-100" />
                                                </div>
                                            </a>
                                        </div></td>
                                            ...
                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                ...
                                                <div class="menu-item px-3">
                                                    <a href="#" class="menu-link px-3 delete_alert"
                                                        data-id="{{ $country->id }}">{{ __('Delete') }}</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
...
<script>
    document.getElementById('confirmExportButton').addEventListener('click', function() {
        fetch('{{ route('admin.countries.export') }}')
            .then(response => {
                if (response.ok) {
...
<script>
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: "{{ url('admin/countries') }}/" + country_id,
                data: {
                    _method: 'DELETE',
...
<script type="text/javascript">
    $(document).ready(function() {
      function fetchCategories(keyword, page = 1) {
          $.ajax({
              url: '{{ route('admin.countries.search') }}?page=' + page,
              type: 'POST',
              data: {
                  _token: '{{ csrf_token() }}',
                  keyword: keyword
              ...
              success: function(response) {
                  var tableBody = $('#kt_table_countries tbody');
                  tableBody.empty();
                  $.each(response.countries.data, function(index, country) {
                    var flagPath = country.flag_path;
    ...
    var row = '<tr>' +
        ...
        '<td class="d-flex align-items-center">' +
        '<div class="d-flex flex-column">' +
        '<a href="/admin/countries/' + country.id + '" class="text-gray-800 text-hover-primary mb-1">' + country.name_ar + '</a>' +
        '<span>' + country.name_en + '</span></div></td>' +
        '<td><div class="symbol symbol-circle symbol-50px overflow-hidden me-3">' +
        '<a href="/admin/countries/' + country.id + '">' +
        '<div class="symbol-label">' +
        '<img src="' + flagPath + '" alt="' + country.name_ar + '" class="w-100" /></div></a></div></td>' +
        '<td>' + country.code + '</td>' +
        '<td class="text-end">' +
        ...
        '<div class="menu-item px-3"><a href="/admin/countries/' + country.id + '/edit" class="menu-link px-3">{{ __('Edit') }}</a></div>' +
        '<div class="menu-item px-3"><a href="#" class="menu-link px-3 delete_alert" data-id="' + country.id + '">{{ __('Delete') }}</a></div>' +
        '</div></td></tr>';
    tableBody.append(row);
      ...
      $(document).on('click', '#pagination a', function(event) {
          ...
          var keyword = $('#search-country').val();
