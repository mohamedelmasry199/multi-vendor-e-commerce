...
<div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
    ...
    <form id="filterForm" action="{{ route('admin.users.filter') }}" method="GET">
        <div class="px-7 py-5">
            <div class="mb-10">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            function fetchUsers(keyword, page = 1) {
                $.ajax({
                    url: '{{ route('admin.users.search') }}?page=' + page,
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        keyword: keyword
                    ...
                    success: function(response) {
                        ...
                        $.each(response.users.data, function(index, user) {
                            var statusText = user.status == 'active' ? '{{ __('Active') }}' :
                                '{{ __('Not Active') }}';
                            var statusClass = user.status == 'active' ? 'badge-light-success' :
                                'badge-light-warning';
                            var enableDisableAction = user.status == "active" ?
                                '<a href="#" class="menu-link px-3 active_alert" data-value="not_active" data-id="' +
                                user.id + '">{{ __('Disable') }}</a>' :
                            ...
                            var row = '<tr>' +
                                ...
                                '</a>' +
                                '<span>' + user.email + '</span></div></td>' +
                                '<td>' + user.phone + '</td>' +
                                '<td>' + (user.city ? user.city.country.name_ar : '') +
                                '</td>' +
                                '<td><div class="badge badge-light fw-bold">' + (user.city ?
                                    user.city.name_ar : '') + '</div></td>' +
                                '<td>' + user.address + '</td>' +
