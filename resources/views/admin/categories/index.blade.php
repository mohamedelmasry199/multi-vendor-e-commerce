                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-title">
                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <!--end::Svg Icon-->
                            <input type="text" id="search-category" class="form-control form-control-solid w-250px ps-14"
...
<div class="d-flex align-items-center">
    ...
    <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
        ...
        <form id="filterForm" action="{{ route('admin.categories.filter') }}" method="GET">
            <div class="px-7 py-5">
                <div class="mb-10">
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_categories">
                            ...
                            <thead>
                                ...
                                <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                                    ...
                                    <th class="min-w-100px">{{ __('Description(AR)') }}</th>
                                    <th class="min-w-100px">{{ __('Parent Category') }}</th>
                                    ...
                                    <tr>
                                        ...
                                        <td class="d-flex align-items-center">
                                            ...
                                            <div class="d-flex flex-column">
                                                <a href="{{ route('admin.categories.show', ['category' => $category->id]) }}"
                                                    class="text-gray-800 text-hover-primary mb-1">{{ $category->name_ar }}</a>
...
<script>
    document.getElementById('confirmExportButton').addEventListener('click', function() {
        fetch('{{ route('admin.categories.export') }}')
            .then(response => {
                if (response.ok) {
...
<script type="text/javascript">
  $(document).ready(function() {
    function fetchCategories(keyword, page = 1) {
        $.ajax({
            url: '{{ route('admin.categories.search') }}?page=' + page,
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                keyword: keyword
            ...
            success: function(response) {
                var tableBody = $('#kt_table_categories tbody');
                tableBody.empty();
                $.each(response.categories.data, function(index, category) {
                    var row = '<tr>' +
                        ...
                        '<div class="symbol-label">' +
                        '<img src="' + category.image + '" alt="' + category.name_ar + '" class="w-100" />' +
                        '</div></a></div>' +
                        '<div class="d-flex flex-column">' +
                        '<a href="/admin/categories/' + category.id + '" class="text-gray-800 text-hover-primary mb-1">' + category.name_ar + '</a>' +
                        '<span>' + category.name_en + '</span></div></td>' +
                        '<td>' + category.description_ar + '</td>' +
                        ...
                        '<div class="menu-item px-3"><a href="/admin/categories/' + category.id + '/edit" class="menu-link px-3">{{ __('Edit') }}</a></div>' +
                        '<div class="menu-item px-3"><a href="#" class="menu-link px-3 delete_alert" data-id="' + category.id + '">{{ __('Delete') }}</a></div>' +
                        '</div></td></tr>';
